...
class StarflightTest {
    ...
    fun markMessagesAsOpened(){
        Starflight.init(SENDER_ID, APP_ID, CLIENT_SECRET)
        register()
        var callback1 = (object : StarFlightCallback<MessageOpenedResponse> {
            override fun onSuccess(result: MessageOpenedResponse) {
                if(result.result == MessageOpenedResponse.Result.OK){
                    successCallbackCalled = true
                }
                else{
                    throw Exception("Wrong result for callback1: ${result.result}")
                }
            }
        ...
        Starflight.markMessageAsOpened(appContext, MESSAGE_UUID, callback1)
        await().atMost(5, TimeUnit.SECONDS).until{successCallbackCalled}

        var callback2 = (object : StarFlightCallback<MessageOpenedResponse> {
            override fun onSuccess(result: MessageOpenedResponse) {
                if(result.result == MessageOpenedResponse.Result.ALREADY_OPENED){
                    successCallbackCalled = true
                }
                else{
                    throw Exception("Wrong result for callback1: ${result.result}")
        ...
        Starflight.markMessageAsOpened(appContext, MESSAGE_UUID, callback2)
        await().atMost(5, TimeUnit.SECONDS).until{successCallbackCalled}
    }
